    ...
    public class ScheduleController : ControllerBase
    {
            ...
            [FromQuery] string? faculty_id = null)
        {
            ...
            var slots = await query
                ...
                {
                    Id = s.Id,
                    Course = s.Section != null && s.Section.Course != null ? new IdNameDto { Id = s.Section.Course.Id, Name = s.Section.Course.Title } : null,
                    Section = s.Section != null ? new IdNameDto { Id = s.Section.Id, Name = s.Section.Name } : null,
                    Instructor = s.Section != null && s.Section.Instructor != null && s.Section.Instructor.User != null 
                        ? new IdNameDto { Id = s.Section.Instructor.UserId, Name = s.Section.Instructor.User.Name } 
                        : null,
        ...
        public async Task<IActionResult> CreateScheduleSlot([FromBody] CreateScheduleSlotRequestDto request)
        {
            ...
            var slot = new ScheduleSlot
            {
                ...
                SectionId = request.CourseSectionId,
        ...
        public async Task<IActionResult> UpdateSlot(string slot_id, [FromBody] UpdateScheduleSlotRequestDto request)
        {
            ...
            if (!string.IsNullOrEmpty(request.CourseSectionId)) slot.SectionId = request.CourseSectionId;
            ...
            [FromQuery] string? section_id = null)
        {
            ...
            if (!string.IsNullOrEmpty(room_id))
            {
                ...
                {
                    roomConflicts.Add(new { type = "room_overlap", slot_id = c.Id, course_name = c.Section?.Course?.Title, room_name = c.Room?.Name });
                }
            }
            ...
            if (!string.IsNullOrEmpty(section_id))
            {
                ...
                if (section != null && !string.IsNullOrEmpty(section.InstructorId))
                {
                    var iConflicts = await _context.ScheduleSlots
                        .Include(s => s.Section)
                            .ThenInclude(sec => sec!.Course)
                        .Where(s => s.Id != slot_id && s.Day == day.Value && s.Section != null && s.Section.InstructorId == section.InstructorId &&
                            ((string.Compare(s.StartTime, start_time) < 0 && string.Compare(s.EndTime, start_time) > 0) ||
                             (string.Compare(s.StartTime, end_time) < 0 && string.Compare(s.EndTime, end_time) > 0) ||
                             (string.Compare(s.StartTime, start_time) >= 0 && string.Compare(s.EndTime, end_time) <= 0)))
                        .ToListAsync();
                    ...
                    {
                        instructorConflicts.Add(new { type = "instructor_busy", slot_id = c.Id, course_name = c.Section?.Course?.Title, instructor_name = c.Section?.Instructor?.UserId });
                    }
                }
            }
