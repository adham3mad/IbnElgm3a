...
namespace IbnElgm3a.Models.Seeder
{
    public static class PermissionSeeder
    {
        ...
        {
            // Reset Features sequence to avoid PK violation if data exists but sequence is out of sync
            try {
                await context.Database.ExecuteSqlRawAsync("SELECT setval(pg_get_serial_sequence('\"Features\"', 'Id'), coalesce(max(\"Id\"), 0) + 1, false) FROM \"Features\";");
            } catch { /* Ignore if not supported/database not ready */ }
